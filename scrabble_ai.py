...
class ScrabbleAI(Player):
    ...
    def play_hand():
        ...
        AI's perspective
        """
        word = self.find_acceptable_word(self, [tile.get_letter() for tile in self.tiles])
        ...
        return word.get_score(), [tile.get_letter() for tile in self.tiles]
    ...
    def find_acceptable_word(self, letters):
        ...
        for coord in self.board.anchor_coords :
    ...
    def get_contiguous_block_down(self, coord):
        ...
        while row < scrabble.MAX_LENGTH and self.board.player_board[row][col] != '' :

            contig_block += self.board.player_board[row][col]
            row += 1
    ...
    def get_contiguous_block_right(self, coord):
        ...
        #loop through in downward direction and add to word if not ''
        while col < scrabble.MAX_LENGTH and self.board.player_board[row][col] != '':
    ...
    def get_contiguous_block_up(self, coord):
        ...
        while row > 0 and self.board.player_board[row][col] != '':

            contig_block = self.board.player_board[row][col] + contig_block
            row -= 1
    ...
    def get_contiguous_block_left(self, coord):
        ...
        while col > 0 and self.board.player_board[row][col] != '':
            contig_block = self.board.player_board[row][col] + contig_block
            col -= 1
        ...
        for letter in letters:
            ...
            try:
                ...
                if col >= end_col:
                    ...
                    score = self.board.get_hand_legality_by_score(modified_letter_dict)
    ...
    def get_word_left(self, row, col, letters):
        ...
        for i in range(0, len(letters)):
            if self.board.player_board[row][col - i]:
                continue
    ...
    def get_word_up(self, row, col, letters):
        ...
        best_word = Word("", {}, 0)
        for i in range(0, len(letters)):
            if self.board.player_board[row-i][col]:
                continue
            word = self.get_word_down(row - i, col, letters, self.get_contiguous_block_up((row, col - i)), {}, row)
            if word > best_word:
                best_word = word
            if best_word.get_score() > self.threshold:
                return best_word
